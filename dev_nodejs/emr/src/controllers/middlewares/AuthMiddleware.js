...
exports.AuthMiddleware = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function () {
        var token, user, wpUserIds;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    token = req.headers.token;
                    if (!token) {
                        if (!Config_1.config.get("testAuthToken", false)) {
                            return [2 /*return*/, res.status(401).send({
                                    message: "Token not provided", success: 0
                                })];
                        }
                        if (Config_1.config.get("testAuthToken")) {
                            token = Config_1.config.get("testAuthToken");
                        }
                    }
                ...
                case 1:
                    ...
                    wpUserIds = [user.user_id];
                    if (user.user_type === 'assistant' && user.meta["doctors"])
                        user.meta["doctors"].forEach(function (doctor) {
